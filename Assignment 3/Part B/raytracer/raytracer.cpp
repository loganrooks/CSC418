...
void Raytracer::antiAliasRender4x(Camera& camera, Scene& scene, LightList& light_list, Image& image) {
	...
	for (int i = 0; i < image.height; i++) {
		for (int j = 0; j < image.width; j++) {
			...
			imagePlane[2] = -1;
			// Chris's contribution begins
			// Supersampling with stochastic ray casting: imagePlane is in "center" of pixel.
			// Add or subtract random*(actual pixelsize)/2 in both the i and j directions
			// to get 4 rays sent into random segments of four quadrants of the pixel.
			Point3D imagePlane1;
			Point3D imagePlane2;
			Point3D imagePlane3;
			Point3D imagePlane4;
			...
			imagePlane1[0] = imagePlane[0] - 1.0/(factor*3.0);
			imagePlane1[1] = imagePlane[1] + 1.0/(factor*3.0);
			imagePlane1[2] = imagePlane[2];

			//top right quadrant:
			imagePlane2[0] = imagePlane[0] + 1.0/(factor*3.0);
			imagePlane2[1] = imagePlane[1] + 1.0/(factor*3.0);
			imagePlane2[2] = imagePlane[2];

			//bottom right quadrant:
			imagePlane3[0] = imagePlane[0] + 1.0/(factor*3.0);
			imagePlane3[1] = imagePlane[1] - 1.0/(factor*3.0);
			...
			Color col = (1.0/3.0) * (shadeRay(ray1, scene, light_list) +
